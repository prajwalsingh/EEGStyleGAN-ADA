...
def setup_snapshot_image_grid(test_set, random_seed=0):
    ...
    gh = np.clip(7680 // test_set.image_shape[2], 8, 32) # For each class 32 samples
    gw = np.clip(5120 // test_set.image_shape[1], 5, 40) # Number of classes 40
    ...
    return (gw, gh), np.stack(images).T, np.stack(labels).T
