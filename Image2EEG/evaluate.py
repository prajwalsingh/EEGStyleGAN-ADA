...
def test(epoch, eeg_model, model, optimizer, loss_fn, miner, test_dataloader, experiment_num):
    ...
    for batch_idx, (eeg, images, labels, c) in enumerate(tq, start=1):
        ...
        # labels_array     = np.concatenate((labels_array, subject.cpu().detach().numpy()), axis=0) if labels_array.size else subject.cpu().detach().numpy()
    ...
    knn_cv = KNeighborsClassifier(n_neighbors=config.num_classes)
...
if __name__ == '__main__':
    ...
    running_test_loss, test_acc   = test(epoch, eeg_model, model, optimizer, loss_fn, miner, test_dataloader, experiment_num)