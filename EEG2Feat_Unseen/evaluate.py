...
def test(epoch, model, optimizer, loss_fn, miner, test_dataloader, experiment_num):
	...
	num_clusters   = 6#config.num_classes
	...
	reduced_embedding = tsne_plot.plot(eeg_featvec_proj, labels_array, clustering_acc_proj, 'test', experiment_num, epoch, proj_type='proj', idx_to_clsidx=idx_to_clsidx, clsidx_to_clsname=clsidx_to_clsname)
	...
	knn_cv = KNeighborsClassifier(n_neighbors=num_clusters)
...
if __name__ == '__main__':
    ...
    train_cluster  = 0
    test_cluster   = 0
    removed_class  = ['n02106662', 'n02124075', 'n03982430', 'n02607072', 'n02951358', 'n03445777']
    clsidx_to_idx  = {}
    idx_to_clsidx  = {}
    clsidx_to_clsname = {'n02106662': 'Dog', 'n02124075': 'Cat', 'n03982430': 'Pool', 'n02607072': 'Fish', 'n02951358': 'Canoe', 'n03445777': 'Golf'}
    new_cls_idx    = 0
    ...
    for i in tqdm(natsorted(glob(base_path + test_path))):
        loaded_array = np.load(i, allow_pickle=True)
        if loaded_array[3].split('_')[0] in removed_class:
            ...
            if loaded_array[2] not in clsidx_to_idx:
                clsidx_to_idx[loaded_array[2]] =  new_cls_idx
                idx_to_clsidx[new_cls_idx] = loaded_array[3].split('_')[0]
                new_cls_idx += 1
            label_test.append(clsidx_to_idx[loaded_array[2]])

    x_test_eeg   = np.array(x_test_eeg)
    x_test_image = np.array(x_test_image)
    test_labels  = np.array(label_test)

    print('Total classes in test: {}, Total EEG: {}'.format(np.unique(test_labels).shape, x_test_eeg.shape))
    ...
    ckpt_lst = natsorted(glob('EXPERIMENT_{}/bestckpt/eegfeat_all_0.9632786885245902.pth'.format(experiment_num)))
