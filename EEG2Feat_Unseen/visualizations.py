...
class TsnePlot:
    ...
    def plot(self, embedding, labels, score, exp_type, experiment_num, epoch, proj_type, idx_to_clsidx=None, clsidx_to_clsname=None):
        ...
        for i, label in enumerate(unique_labels):
            mask = labels == label
            if idx_to_clsidx != None:
                ax.scatter(reduced_embedding[mask, 0], reduced_embedding[mask, 1], c=colors[i], label=clsidx_to_clsname[idx_to_clsidx[label]], alpha=0.6)
            else:
