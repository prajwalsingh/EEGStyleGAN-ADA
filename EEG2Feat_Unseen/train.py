...
if __name__ == '__main__':
    ...
    for i in tqdm(natsorted(glob(base_path + train_path))):
        ...
        if loaded_array[3].split('_')[0] not in removed_class:
            ...
            if loaded_array[2] not in clsidx_to_idx:
                ...
                new_cls_idx += 1
            labels.append(clsidx_to_idx[loaded_array[2]])
    ...
    for i in tqdm(natsorted(glob(base_path + validation_path))):
        loaded_array = np.load(i, allow_pickle=True)
        if loaded_array[3].split('_')[0] not in removed_class:
            ...
            if loaded_array[2] not in clsidx_to_idx:
                clsidx_to_idx[loaded_array[2]] =  new_cls_idx
                new_cls_idx += 1
            label_Val.append(clsidx_to_idx[loaded_array[2]])
    ...
    print('Total classes in val: {}, Total EEG: {}'.format(np.unique(val_labels).shape, x_val_eeg.shape))
