...
if __name__ == '__main__':
    ...
    test_path = config.validation_path
    ...
    for i in tqdm(natsorted(glob(base_path + train_path))):
        ...
        if loaded_array[3].split('_')[0] in removed_class:
            ...
            if loaded_array[2] not in clsidx_to_idx:
                ...
                new_cls_idx += 1
            label_train.append(clsidx_to_idx[loaded_array[2]])
    x_train_eeg   = np.array(x_train_eeg)
    x_train_image = np.array(x_train_image)
    train_labels  = np.array(label_train)

    print('Total classes in train: {}, Total EEG: {}'.format(np.unique(train_labels).shape, x_train_eeg.shape))
    ...
    cv_scores = cross_val_score(knn_cv, X_eeg_test, Y_test, cv=5)#print each cv score (accuracy) and average them
    print(cv_scores)
